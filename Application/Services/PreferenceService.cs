...
public class PreferenceService : IPreferenceService
{
    ...
    public async Task<PreferenceResponseDTO> Add(PreferenceAddDTO dto)
    {
        ...
        foreach (var c in (dto.castDTO ?? new List<CastAddDTO>())
        ...
        foreach (var c in (dto.crewDTO ?? new List<CrewAddDTO>())
                 .GroupBy(c => c.id).Select(g => g.First())
                 .Where(c => !string.IsNullOrWhiteSpace(c.name)))
        {
        ...
        foreach (var g in (dto.genreDTO ?? new List<GenreAddDTO>())
        ...
        foreach (var s in (dto.streamingDTO ?? new List<StreamingAddDTO>())
